...
interface AccountManagementContextType {
  ...
  // ChaosStar CLI Info
  chaosStarSubnets: any[];
  networkStatus: any;
  chaosStarKeys: any[];
  loadingChaosStar: boolean;
  loadChaosStarInfo: () => Promise<void>;
...
export function AccountManagementProvider({ children }: { children: ReactNode }) {
  ...
  const [chaosStarKeys, setChaosStarKeys] = useState<any[]>([]);
  const [loadingChaosStar, setLoadingChaosStar] = useState(false);
  ...
  const loadAccounts = async () => {
    ...
    try {
      ...
      try {
        ...
        if (hasAccountRegistryContract()) {
          ...
          // Also load accounts for all ChaosStar keys
          ...
          // Get accounts for all addresses (connected wallet + ChaosStar keys)
          ...
          loadedAccounts = accountResults
            ...
            .map((acc: any) => {
              ...
              // Find which ChaosStar key this account belongs to (if any)
              const walletAddr = acc.walletAddress || acc.ownerWallet;
              const matchingKey = chaosStarKeys.find(
    ...
    try {
      const result = await accountApi.listAccounts(address);
          ...
          // Suppress timeout and connection errors - backend might not be running
          const isConnectionError = apiError.name === "AbortError" || 
      ...
      if (isMainFundedAccount && !mainFundedExists) {
        try {
          await accountApi.createAccount({
            ...
            description: "Primary funded account from ChaosStar CLI",
          });
  ...
  const loadxBGLBalance = async () => {
    ...
    try {
      // Try using ChaosStar SDK ChainKit first (better indexed data)
      try {
        const { getEnhancedBalances } = await import("@/lib/chaosstar-sdk");
      ...
      if (!CONTRACT_ADDRESSES.csnToken || CONTRACT_ADDRESSES.csnToken.trim() === "") {
        console.debug("xBGL token contract address not available");
        setCsnBalance(null);
        return;
      }
      try {
      ...
      console.debug("xBGL balance loaded:", formatted);
      } catch (contractError: any) {
        console.debug("Failed to fetch xBGL balance from contract:", contractError.message);
        // Don't throw - just leave balance as null
      }
    } catch (error: any) {
      console.error("Failed to load xBGL balance:", error);
  ...
  useEffect(() => {
    if (address && isConnected) {
      loadChaosStarInfo().then(() => {
        // Load accounts after ChaosStar keys are loaded so we can check all key addresses
      loadAccounts();
      });
      loadClusters();
      loadTransfers();
