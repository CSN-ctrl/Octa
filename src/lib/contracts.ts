...
const getContractAddresses = () => {
  ...
  const addresses = {
    ...
    xbglToken: import.meta.env.VITE_XBGL_TOKEN_ADDRESS || "0x868306CeD3bb5Aa8fBc4BD8fA2727484cDfE1D89", // xBGL Token (Wrapped Native)
  ...
  // Fallback: Use known deployed PlotRegistry address if not loaded
  if (!addresses.plotRegistry || addresses.plotRegistry.trim() === "") {
    addresses.plotRegistry = "0x648AFD1beBCfd9D4A25Ad8EF81f59eF153C5aDBB";
  }
...
// ERC1155 PlotRegistry ABI (matches deployed contract at 0x648AFD1beBCfd9D4A25Ad8EF81f59eF153C5aDBB)
export const PLOT_REGISTRY_ABI = [
  ...
  // PlotRegistry specific functions
  "function totalPlots() view returns (uint256)",
  ...
  "function getPlotIdByCoordinates(uint32 x, uint32 y) view returns (uint256)",
  "function getPlotsByOwner(address owner) view returns (uint256[])",
  "function getPlotCountByOwner(address owner) view returns (uint256)",
  "function plotExists(uint32 x, uint32 y) view returns (bool)",
  "function isPlotPurchased(uint256 plotId) view returns (bool)",
  "function calculatePlotId(uint32 x, uint32 y) pure returns (uint256)",
  "function getCoordinatesFromPlotId(uint256 plotId) pure returns (uint32 x, uint32 y, bool valid)",
  "function batchPlotExists(uint32[] calldata coordinates) view returns (bool[])",
  "function batchGetPlotMetadata(uint256[] calldata plotIds) view returns ((uint32,uint32,uint8,bool,uint256,bytes32)[])",
  // Events
  "event PlotIssued(uint256 indexed plotId, uint32 indexed x, uint32 indexed y, uint256 price, uint8 level)",
  "event TransferSingle(address indexed operator, address indexed from, address indexed to, uint256 id, uint256 value)",
  "event TransferBatch(address indexed operator, address indexed from, address indexed to, uint256[] ids, uint256[] values)",
];
...
// The xbglToken address in CONTRACT_ADDRESSES is for reference only
// Native xBGL balance is accessed via provider.getBalance(address)
export function hasXBGLTokenContract(): boolean {
  // xBGL is native, so this always returns true if we have an RPC connection
  return true;
}
