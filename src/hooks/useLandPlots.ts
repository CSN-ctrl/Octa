...
export function useLandPlots() {
  ...
  const [totalPlots, setTotalPlots] = useState<bigint>(10000n); // 10,000 plots total
  const [priceInNative, setPriceInNative] = useState<bigint>(0n);
  ...
  const fetchLandData = useCallback(async () => {
    ...
    try {
      ...
      if (contractProvider) {
        try {
          const code = await withTimeout(contractProvider.getCode(contract.target), 5000);
          if (!code || code === "0x") {
      ...
      try {
        const [soldResult, priceResult, activeResult] = await Promise.all([
          ...
          withTimeout(contract.priceInAVAX(), 10000), // Contract uses AVAX naming but this is xBGL on ChaosStar
      ...
      */
      ...
      if (isRpcError) {
        ...
        console.debug("RPC node not available for land data (expected if local node isn't running)");
        setUserPlots([]);
        setPendingPlots([]);
      } else {
  ...
  const buyPlot = async (plotId: number) => {
    ...
    try {
      ...
      const tx = await contract.buyPlot(plotId, { value: priceInNative });
      await tx.wait();
  ...
  const buyPlotPhase1xBGL = async (plotId: number) => {
    if (!signer) throw new Error("Wallet not connected");
    if (!hasLandContract()) {
      throw new Error("Land contract address not set");
    }
    ...
    try {
      ...
      if (signerProvider) {
        ...
        console.log("Purchasing plot with xBGL on network:", network);
      }

      const contract = getLandContract(signer);
      // Plot price is fixed at 100 xBGL
      const PLOT_PRICE_xBGL = ethers.parseEther("100");

      // Buy plot with native xBGL (100 xBGL per plot)
      // The contract's buyPlotWithAVAX function accepts native coin (xBGL on Chaos Star Network)
      const tx = await contract.buyPlotWithAVAX(plotId, { value: PLOT_PRICE_xBGL });
      const receipt = await tx.wait();
      toast.success(`Payment received (100 xBGL) for plot ${plotId}. Pending activation.`);
      // Wait a bit for the transaction to be fully processed, then refresh
      setTimeout(async () => {
        await fetchLandData();
      }, 2000);
      ...
      console.error("Error buying plot (Phase1 xBGL):", error);
      toast.error(error.reason || error.message || "Failed to pay with xBGL. Ensure you're using an ChaosStar Key connected to Chaos Star Network.");
      throw error;
    } finally {
      setLoading(false);
    }
  ...
  const buyPlotsBatch = async (plotIds: number[]) => {
    ...
    try {
      const contract = getLandContract(signer);
      // Plot price is fixed at 100 xBGL per plot
      const PLOT_PRICE_xBGL = ethers.parseEther("100");
      const totalPrice = PLOT_PRICE_xBGL * BigInt(plotIds.length);
      const tx = await contract.buyPlotsBatch(plotIds, { value: totalPrice });
      const receipt = await tx.wait();

      toast.success(`${plotIds.length} plots purchased successfully for ${plotIds.length * 100} xBGL!`);
      // Wait a bit for the transaction to be fully processed, then refresh
      setTimeout(async () => {
        await fetchLandData();
      }, 2000);
  ...
  return {
    ...
    priceInAVAX: ethers.formatEther(priceInNative),
