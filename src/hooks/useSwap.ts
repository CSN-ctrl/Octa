...
export function useSwap() {
  ...
  // Get quote for swapping xBGL to a token
  ...
  // Swap xBGL (native) for a token
  const swapxBGLForToken = useCallback(async (
    amountIn: string,
    tokenOutAddress: string,
    minAmountOut: string,
    ...
    try {
      ...
      if (balance < amountInWei) {
        throw new Error("Insufficient xBGL balance");
      }
      ...
      throw new Error("DEX integration not yet deployed. Please use a DEX directly to swap xBGL for tokens.");

      // await tx.wait();
      // toast.success(`Successfully swapped ${amountIn} xBGL for tokens!`);
      // return tx.hash;
    } catch (error: any) {
  ...
  // Swap token for xBGL (native)
  const swapTokenForxBGL = useCallback(async (
    ...
    try {
      ...
      throw new Error("DEX integration not yet deployed. Please use a DEX directly to swap tokens for xBGL.");
      ...
      // await tx.wait();
      // toast.success(`Successfully swapped tokens for ${minAmountOut} xBGL!`);
      // return tx.hash;
    } catch (error: any) {
      console.error("Swap error:", error);
      toast.error(error.message || "Swap failed");
